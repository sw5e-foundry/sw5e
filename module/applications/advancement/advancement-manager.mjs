...
export default class AdvancementManager extends Application {
  ...
  get title() {
    ...
    const step = visibleIndex < 0 ? "" : game.i18n.format("SW5E.AdvancementManagerSteps", {
  ...
  static forNewItem(actor, itemData, options={}) {
    ...
    if ( clonedItem.type === "archetype" ) targetLevel = clonedItem.class?.system.levels ?? 0;
  ...
  createLevelChangeSteps(classItem, levelDelta) {
    ...
    for ( let offset = 1; offset <= levelDelta; offset++ ) {
      ...
      pushSteps(this.constructor.flowsForLevel(speciesItem, characterLevel), stepData);
    ...
    for ( let offset = 0; offset > levelDelta; offset-- ) {
      ...
      pushSteps(this.constructor.flowsForLevel(speciesItem, characterLevel).reverse(), stepData);
   ...
   * @param {Item5e} item    Item being advanced. If class or archetype, its level will be used.
  ...
  getData() {
    ...
    return {
      ...
      header: item.name,
      subheader: level ? game.i18n.format("SW5E.AdvancementLevelHeader", { level }) : "",
      steps: {
        current: visibleIndex + 1,
        total: visibleSteps.length,
  ...
  render(...args) {
     ...
     * @function sw5e.preAdvancementManagerRender
     * @memberof hookEvents
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new Dialog({
        title: `${game.i18n.localize("SW5E.AdvancementManagerCloseTitle")}: ${this.actor.name}`,
        content: game.i18n.localize("SW5E.AdvancementManagerCloseMessage"),
        buttons: {
          close: {
            icon: '<i class="fas fa-times"></i>',
            label: game.i18n.localize("SW5E.AdvancementManagerCloseButtonStop"),
            callback: () => super.close(options)
          },
          continue: {
            icon: '<i class="fas fa-chevron-right"></i>',
            label: game.i18n.localize("SW5E.AdvancementManagerCloseButtonContinue")
          }
        },
        default: "close"
      }).render(true);
  ...
  async _restart(event) {
    const restart = await Dialog.confirm({
      title: game.i18n.localize("SW5E.AdvancementManagerRestartConfirmTitle"),
      content: game.i18n.localize("SW5E.AdvancementManagerRestartConfirm")
    });
    if ( !restart ) return;
  ...
  async _complete(event) {
     ...
     * are applied.
     * @function sw5e.preAdvancementManagerComplete
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("sw5e.preAdvancementManagerComplete", this, updates, toCreate, toUpdate, toDelete) === false ) {
     ...
     * @function sw5e.advancementManagerComplete
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager that just completed
     */
    Hooks.callAll("sw5e.advancementManagerComplete", this);
