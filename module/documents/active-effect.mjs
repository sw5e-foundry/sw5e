...
export default class ActiveEffect5e extends ActiveEffect {
  ...
  static async fromStatusEffect(effectData, options={}) {
    ...
    const createData = {
      ...
      _id: staticID(`sw5e${effectData.id}`),
  ...
  _prepareExhaustionLevel() {
    ...
    this.icon = `systems/sw5e/icons/svg/statuses/exhaustion-${level}.svg`;
  ...
  static onTokenHUDRender(app, html) {
    ...
    if ( Number.isFinite(level) && (level > 0) ) {
      html.find('[data-status-id="exhaustion"]').css({
        ...
        background: `url('systems/sw5e/icons/svg/statuses/exhaustion-${level}.svg') no-repeat center / contain`
      });
    }
  }
  ...
  static onManageActiveEffect(event, owner) {
    foundry.utils.logCompatibilityWarning(
      "ActiveEffects5e#onManageActiveEffect has been deprecated in favor of the new sw5e-effects element.",
    ...
    switch (a.dataset.action) {
      case "create":
        ...
        return owner.createEmbeddedDocuments("ActiveEffect", [
          {
            name: isActor ? game.i18n.localize("SW5E.EffectNew") : owner.name,
