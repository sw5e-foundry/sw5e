...
export class ScaleValueAdvancement extends Advancement {
     ...
     */
    static TYPES = {
        string: "SW5E.AdvancementScaleValueTypeString",
        number: "SW5E.AdvancementScaleValueTypeNumber",
        dice: "SW5E.AdvancementScaleValueTypeDice",
        distance: "SW5E.AdvancementScaleValueTypeDistance"
    };
     ...
     */
    valueForLevel(level) {
        const key = Object.keys(this.data.configuration.scale)
            .reverse()
    ...
    formatValue(level) {
        ...
        const value = this.valueForLevel(level);
        if (value == null) return null;
        return `${value.value} ${CONFIG.SW5E.movementUnits[this.data.configuration.distance.units]}`;
    }
}
...
export class ScaleValueConfig extends AdvancementConfig {
    ...
    getData() {
        ...
        data.previewIdentifier =
            config.identifier ||
            this.advancement.data.title?.slugify() ||
            this.advancement.constructor.metadata.title.slugify();
        data.typeHint = game.i18n.localize(`SW5E.AdvancementScaleValueTypeHint${config.type.capitalize()}`);
        ...
        data.movementUnits = CONFIG.SW5E.movementUnits;
        return data;
    }
    ...
    _prepareLevelData() {
        let lastValue = null;
        return Array.fromRange(this.item.maxAdvancementLevel + 1)
            .slice(1)
            .reduce((obj, level) => {
    ...
    async _updateObject(event, formData) {
        ...
        if (typeChange && formData["data.configuration.type"] !== this.advancement.data.configuration.type) {
            ...
            for (const l of Array.fromRange(this.item.maxAdvancementLevel, 1)) {
