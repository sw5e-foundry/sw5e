...
export default class Item5e extends SystemDocumentMixin(Item) {
  ...
  get scaleValues() {
    ...
    const level = this.type === "class" ? this.system.levels : this.type === "archetype" ? this.class?.system.levels
  ...
  get powercasting() {
    ...
    const classSC = isArchetype ? this.class?.system.powercasting : powercasting;
    const archetypeSC = isArchetype ? powercasting : this.archetype?.system.powercasting;
    const finalSC = foundry.utils.deepClone(
      ( archetypeSC && (archetypeSC.progression !== "none") ) ? archetypeSC : classSC
    );
    ...
    if ( CONFIG.SW5E.powercastingTypes[finalSC.progression] ) finalSC.type = finalSC.progression;
    else finalSC.type = Object.entries(CONFIG.SW5E.powercastingTypes).find(([type, data]) => {
  ...
  _prepareActivation() {
    ...
    if ( this.hasTarget ) {
      ...
      if ( this.hasAreaTarget ) {
        if ( tgt.units in C.movementUnits ) target.push(game.i18n.localize(`SW5E.Dist${tgt.units.capitalize()}Abbr`));
    ...
    if ( this.isActive && rng.units ) {
      ...
      if ( rng.units in C.movementUnits ) {
        this.labels.range.push(game.i18n.localize(`SW5E.Dist${rng.units.capitalize()}Abbr`));
      }
  ...
  _prepareRecovery() {
    ...
    this.labels.recovery = CONFIG.SW5E.limitedUsePeriods[per];
    if ( per === "lr" ) this.labels.recovery = game.i18n.localize("SW5E.AbbreviationLR");
    else if ( per === "sr" ) this.labels.recovery = game.i18n.localize("SW5E.AbbreviationSR");
  }
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.SW5E.maxLevel + 1, minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
  ...
  getSaveDC() {
    ...
    if ( save.scaling === "power" ) {
      save.dc = this.isOwned ? this.actor.system.attributes.powerdc : null;
    }
  ...
  prepareDurationValue() {
    ...
    if ( !value ) {
      if ( duration?.units ) this.labels.duration = CONFIG.SW5E.timePeriods[duration.units];
      return;
    }
    ...
    if ( item.type === "power" ) {
      ...
      if ( config.slotLevel ) {
        ...
        level = Number.isInteger(config.slotLevel) ? config.slotLevel
          : config.slotLevel === "pact" ? as.powers.pact.level : parseInt(config.slotLevel.replace("power", ""));
  ...
  _getUsageConfig() {
    ...
    if ( scaling === "slot" ) {
      ...
      config.slotLevel = (preparation?.mode === "pact") ? "pact" : `power${level}`;
  ...
  _getUsageUpdates(config) {
    ...
    if ( config.consumePowerSlot ) {
      ...
      if ( powers === 0 ) {
        const labelKey = config.slotLevel === "pact" ? "SW5E.PowerProgPact" : `SW5E.PowerLevel${this.system.level}`;
        const label = game.i18n.localize(labelKey);
        ui.notifications.warn(game.i18n.format("SW5E.PowerCastNoSlots", {name: this.name, level: label}));
        return false;
      }
      actorUpdates[`system.powers.${config.slotLevel}.value`] = Math.max(powers - 1, 0);
    }
  ...
  async rollAttack(options={}) {
    ...
    const elvenAccuracy = (flags.elvenAccuracy
      && CONFIG.SW5E.characterFlags.elvenAccuracy.abilities.includes(this.abilityMod)) || undefined;
  ...
  async rollDamage({critical, event=null, powerLevel=null, versatile=false, options={}}={}) {
    ...
    if ( this.type === "power" ) {
      if ( scaling.mode === "at-will" ) {
        ...
        rollConfigs.forEach(c => this._scaleAt-WillDamage(c.parts, scaling.formula, level, rollData));
      }
      else if ( powerLevel && (scaling.mode === "level") && scaling.formula ) {
        rollConfigs.forEach(c =>
          this._scalePowerDamage(c.parts, this.system.level, powerLevel, scaling.formula, rollData)
        );
      }
    }
    ...
    try {
      ...
      switch ( action ) {
        case "applyEffect":
          ...
          if ( warn ) ui.notifications.warn("SW5E.EffectApplyWarning", { localize: true });
          break;
        case "attack":
  ...
  _onCreateOwnedPower(data, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "system.preparation.prepared") === undefined ) {
      updates["system.preparation.prepared"] = isNPC; // NPCs automatically prepare powers
    }
    return updates;
  }
  ...
  async deleteDialog(options={}) {
    ...
    if ( this.isEmbedded && (this.actor.type !== "group") && !game.settings.get("sw5e", "disableAdvancements") ) {
  ...
  static async createScrollFromPower(power, options={}) {
    ...
    const desc = [
      ...
      isConc ? `<p><em>${game.i18n.localize("SW5E.ScrollRequiresConcentration")}</em></p>` : null,
      "<hr>",
      description.value,
      "<hr>",
      `<h3>${game.i18n.localize("SW5E.ScrollDetails")}</h3>`,
      "<hr>",
      scrollDetails
    ].filterJoin("");
