...
async function enrichAward( config, label, options ) {
  ...
  block.classList.add( "award-block", "sw5e2" );
  ...
  block.innerHTML += `
    ...
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize( "SW5E.Award.Action" )}
    </a>
  `;
 ...
 *   <a class="content-link" draggable="true"
 *      data-uuid="Compendium.sw5e.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
 ...
 *      data-tooltip="EDITOR.SW5E.Inline.ApplyStatus" aria-label="Apply Status to Selected Tokens">
...
async function enrichReference( config, label, options ) {
  ...
  if ( isCondition && ( config.apply !== false ) ) {
    ...
    apply.dataset.tooltip = "EDITOR.SW5E.Inline.ApplyStatus";
...
export function createRollLabel( config ) {
  const { label: ability, abbreviation } = CONFIG.SW5E.abilities[config.ability] ?? {};
  const skill = CONFIG.SW5E.skills[config.skill]?.label;
...
function createRollLink( label, dataset ) {
  ...
  if ( game.user.isGM && ( dataset.type !== "damage" ) && ( dataset.type !== "item" ) ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.SW5E.Inline.RequestRoll";
