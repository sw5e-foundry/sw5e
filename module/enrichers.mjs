...
async function enrichCheck(config, label, options) {
  ...
  const skillConfig = CONFIG.SW5E.enrichmentLookup.skills[config.skill];
  ...
  const toolUUID = CONFIG.SW5E.enrichmentLookup.tools[config.tool];
  ...
  let abilityConfig = CONFIG.SW5E.enrichmentLookup.abilities[config.ability];
  ...
  if ( !label ) {
    ...
    if ( config.passive ) {
      ...
      if ( config.dc ) label = game.i18n.format("EDITOR.SW5E.Inline.DC", { dc: config.dc, check: label });
...
async function enrichDamage(config, label, options) {
  ...
  const localizationData = {
    ...
    type: game.i18n.localize(CONFIG.SW5E.damageTypes[config.damageType] ?? "").toLowerCase()
  };
  ...
  span.innerHTML = game.i18n.format(`EDITOR.SW5E.Inline.Damage${localizationType}`, localizationData);
  return span;
}
...
async function enrichSave(config, label, options) {
  ...
  if ( !label ) {
    ...
    if ( config.dc ) label = game.i18n.format("EDITOR.SW5E.Inline.DC", { dc: config.dc, check: label });
    label = game.i18n.format(`EDITOR.SW5E.Inline.Save${config.format === "long" ? "Long" : "Short"}`, {
      save: label
    });
  }
...
function rollAction(event) {
  ...
  switch ( type ) {
    ...
    default:
      return console.warn(`SW5e | Unknown roll type ${type} provided.`);
  }
}
...
async function rollDamage(event, speaker) {
  ...
  const messageData = { "flags.sw5e.roll.type": "damage", speaker };
  const rollConfig = {
    parts: [formula],
    flavor: `${title} (${game.i18n.localize(CONFIG.SW5E.damageTypes[damageType] ?? damageType)})`,
    event,
    title,
    messageData
  };
