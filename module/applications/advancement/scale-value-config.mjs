...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      previewIdentifier:
        config.identifier || this.advancement.title?.slugify() || this.advancement.constructor.metadata.title.slugify(),
      type: type.metadata,
  ...
  _prepareLevelData() {
    let lastValue = null;
    let levels = Array.fromRange(CONFIG.SW5E.maxLevel + 1);
    if ( ["class", "archetype"].includes(this.advancement.parent.type) ) levels = levels.slice(1);
  ...
  _onIdentifierHintCopy(event) {
    ...
    game.tooltip.activate(event.target, { text: game.i18n.localize("SW5E.IdentifierCopied"), direction: "UP" });
  }
  ...
  async _updateObject(event, formData) {
    ...
    if (typeChange && updates.configuration.type !== this.advancement.configuration.type) {
      ...
      await this.advancement.update(
        Array.fromRange(CONFIG.SW5E.maxLevel, 1).reduce((obj, lvl) => {
