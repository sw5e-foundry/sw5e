...
export default class Tooltips5e {
  ...
  async _onHoverContentLink(doc) {
    // TODO SW5E: Remove the next line once rules are fixed
    if ( !doc ) return;
  ...
  async _onHoverPassive(skill, dc) {
    const config = CONFIG.SW5E.skills[skill];
    const label = game.i18n.format("SW5E.SkillPassiveHint", { skill: config.label });
