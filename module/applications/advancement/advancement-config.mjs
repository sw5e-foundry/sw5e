...
export default class AdvancementConfig extends FormApplication {
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      ui.notifications.error("SW5E.AdvancementItemGrantRecursiveWarning", {localize: true});
      return null;
    }

    // Abort if this uuid exists already
    if ( existingItems.find(i => i.uuid === item.uuid) ) {
      ui.notifications.warn("SW5E.AdvancementItemGrantDuplicateWarning", {localize: true});
      return null;
    }
