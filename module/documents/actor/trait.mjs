 ...
 * @param {string} trait  Trait as defined in `CONFIG.SW5E.traits`.
 ...
 * @param {string} trait  Trait as defined in `CONFIG.SW5E.traits`.
 * @returns {object}      Trait categories defined within `CONFIG.SW5E`.
 */
export function categories(trait) {
  const traitConfig = CONFIG.SW5E.traits[trait];
  return CONFIG.SW5E[traitConfig.configKey ?? trait];
}
...
export async function choices(trait, chosen=new Set()) {
  ...
  if ( traitConfig.children ) {
    for ( const [categoryKey, childrenKey] of Object.entries(traitConfig.children) ) {
      const children = CONFIG.SW5E[childrenKey];
  ...
  if ( traitConfig.subtypes ) {
    ...
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      if ( CONFIG.SW5E[key] ) Object.assign(obj, CONFIG.SW5E[key]);
      return obj;
    }, {});
  ...
  // Sort Categories
  if ( traitConfig.sortCategories ) data = sw5e.utils.sortObjectEntries(data, "label");
...
export function traitLabel(trait, count) {
  ...
  return game.i18n.localize(`SW5E.Trait${typeCap}Plural.${pluralRule}`);
}
...
export function keyLabel(trait, key) {
  ...
  for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
    if ( CONFIG.SW5E[childrenKey]?.[key] ) return CONFIG.SW5E[childrenKey]?.[key];
  }

  for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
    if ( !CONFIG.SW5E[idsKey]?.[key] ) continue;
    const index = getBaseItem(CONFIG.SW5E[idsKey][key], { indexOnly: true });
...
export function choiceLabel(trait, choice) {
  ...
  if ( !choice.pool ) {
    return game.i18n.format("SW5E.TraitConfigChooseAny", {
      count: choice.count,
  ...
  return game.i18n.format("SW5E.TraitConfigChooseList", {
    count: choice.count,
