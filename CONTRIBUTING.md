# Contributing to foundrynet/sw5e
...
Cloning this repository and either placing it in or symlinking it to your `Data/systems/sw5e` user data directory is all that is necessary to run this within Foundry VTT. However, if you want to make changes to either the LESS stylesheets or the compendia, there are some developer tools which will make your life easier.
...
Converts the LESS in `./less` to the final `sw5e.css`.
...
Non-English languages are not contained within the core sw5e system, but instead they are managed by specialized [localization modules](https://foundryvtt.com/packages/tag/translation).
...
```text
https://gitlab.com/foundrynet/sw5e/-/releases/release-1.4.3/downloads/sw5e-release-1.4.3.zip
                                              └─ Tag Name ──┘               └─ Tag Name ──┘
```
...
```text
https://gitlab.com/api/v4/projects/foundrynet%2Fsw5e/packages/generic/sw5e/latest/system.json
```
