...
export class SummonsData extends foundry.abstract.DataModel {
  ...
  async summon(profileId) {
     ...
     */
    if ( Hooks.call("sw5e.preSummon", this.item, profile) === false ) return;
    ...
    try {
      ...
      for ( const placement of placements ) {
         ...
         */
        if ( Hooks.call("sw5e.preSummonToken", this.item, profile, tokenUpdateData) === false ) continue;
         ...
         */
        Hooks.callAll("sw5e.summonToken", this.item, profile, tokenData);
     ...
     */
    Hooks.callAll("sw5e.postSummon", this.item, profile, createdTokens);
  }
  ...
  async fetchActor(uuid) {
    ...
    const localActor = game.actors.find(a =>
      a.getFlag("sw5e", "summonedCopy") && (a.getFlag("core", "sourceId") === uuid)
    );
  ...
  async getChanges(actor, profile) {
      ...
      if ( acBonus.total ) {
        if ( actor.system.attributes.ac.calc === "flat" ) {
          ...
          updates.effects.push((new ActiveEffect({
            ...
            name: game.i18n.localize("SW5E.Summoning.ArmorClass.Label")
          })).toObject());
        }
      }
    }
      ...
      if ( hpBonus.total ) {
        if ( (actor.type === "pc") && !actor._source.system.attributes.hp.max ) {
          updates.effects.push((new ActiveEffect({
            ...
            name: game.i18n.localize("SW5E.Summoning.HitPoints.Label")
          })).toObject());
        } else {
    ...
    for ( const item of actor.items ) {
      ...
      if ( this.match.saves && item.hasSave ) {
        itemUpdates["system.save.dc"] = rollData.item.save.dc ?? rollData.attributes.powerdc;
  ...
  async getTokenData({ actor, placement, tokenUpdates, actorUpdates }) {
    ...
    tokenDocument.updateSource({
      "flags.sw5e.summon": { origin: this.item.uuid }
    });
