...
export default class ActiveEffect5e extends ActiveEffect {
  ...
  determineSuppression() {
    ...
    // Deliberately avoiding using fromUuidSync here, see: https://github.com/foundryvtt/sw5e/pull/1980
    const parsed = game.sw5e.isV10 ? _parseUuid(this.origin) : parseUuid(this.origin);
    if ( !parsed ) return;
    ...
    switch ( a.dataset.action ) {
      case "create":
        ...
        return owner.createEmbeddedDocuments("ActiveEffect", [{
          label: isActor ? game.i18n.localize("SW5E.EffectNew") : owner.name,
  ...
  static prepareActiveEffectCategories(effects) {
    ...
    for ( let e of effects ) {
      if ( game.sw5e.isV10 ) e._getSourceName(); // Trigger a lookup for the source name
