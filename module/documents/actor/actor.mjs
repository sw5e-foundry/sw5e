...
export default class Actor5e extends SystemDocumentMixin(Actor) {
  ...
  *allApplicableEffects() {
  ...
  _preparePowercasting() {
    ...
    if ( this.type === "npc" ) {
      if ( progression.slot || progression.pact ) this.system.details.powerLevel = progression.slot || progression.pact;
      else progression.slot = this.system.details.powerLevel ?? 0;
    }
  ...
  static prepareLeveledSlots(powers, actor, progression) {
    const levels = Math.clamp(progression.slot, 0, CONFIG.SW5E.maxLevel);
  ...
  /**
   * Prepare non-leveled power slots using progression data.
   * @param {object} powers        The `data.powers` object within actor's data. *Will be mutated.*
   * @param {Actor5e} actor        Actor for whom the data is being prepared.
   * @param {object} progression   Powercasting progression data.
   * @param {string} key           The internal key for these power slots on the actor.
   * @param {object} table         The table used for determining the progression of slots.
   */
  static prepareAltSlots(powers, actor, progression, key, table) {
    // Power data:
    // - x.level: Slot level for casting
    // - x.max: Total number of slots
    // - x.value: Currently available slots
    // - x.override: Override number of available power slots

    let keyLevel = Math.clamp(progression[key], 0, CONFIG.SW5E.maxLevel);
    powers[key] ??= {};
    const override = Number.isNumeric(powers[key].override) ? parseInt(powers[key].override) : null;

    // Slot override
    if ( (keyLevel === 0) && (actor.type === "npc") && (override !== null) ) {
      keyLevel = actor.system.details.powerLevel;
    }

    const [, keyConfig] = Object.entries(table).reverse().find(([l]) => Number(l) <= keyLevel) ?? [];
    if ( keyConfig ) {
      powers[key].level = keyConfig.level;
      if ( override === null ) powers[key].max = keyConfig.slots;
      else powers[key].max = Math.max(override, 1);
      powers[key].value = Math.min(powers[key].value, powers[key].max);
    }

    else {
      powers[key].max = override || 0;
      powers[key].level = (powers[key].max > 0) ? 1 : 0;
    }
  }
  ...
  static preparePactSlots(powers, actor, progression) {
    this.prepareAltSlots(powers, actor, progression, "pact", CONFIG.SW5E.pactCastingProgression);
  }
  ...
  /**
   * Take a short rest, possibly spending hit dice and recovering resources, item uses, and relevant power slots.
