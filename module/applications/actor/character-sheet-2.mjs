import ActorSheetOrig5eCharacter from "./character-orig-sheet.mjs";
 ...
 */
export default class ActorSheet5eCharacter2 extends ActorSheetOrig5eCharacter {
  constructor(object, options={}) {
    const { width, height } = game.user.getFlag("sw5e", "sheetPrefs.character") ?? {};
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      dragDrop: [
        ...
        { dragSelector: ".powers-list .power-header, .slots[data-favorite-id]", dropSelector: null }
      ],
   ...
   */
  static TABS = [
    { tab: "details", label: "SW5E.Details", icon: "fas fa-cog" },
    { tab: "inventory", label: "SW5E.Inventory", svg: "backpack" },
    { tab: "features", label: "SW5E.Features", icon: "fas fa-list" },
    { tab: "powers", label: "TYPES.Item.powerPl", icon: "fas fa-book" },
    { tab: "effects", label: "SW5E.Effects", icon: "fas fa-bolt" },
    { tab: "biography", label: "SW5E.Biography", icon: "fas fa-feather" }
  ];
  ...
  /** @override */
  get template() {
    return "systems/sw5e/templates/actors/origActor/character-sheet-2.hbs";
  }
  ...
  async getData(options) {
    ...
    // Size
    context.size = {
      label: CONFIG.SW5E.actorSizes[traits.size].label,
      abbr: CONFIG.SW5E.actorSizes[traits.size].abbreviation,
    ...
    context.creatureType = {
      ...
      reference: CONFIG.SW5E.creatureTypes[details.type.value]?.reference,
      subtitle: details.type.subtype
    };

    if ( details.species instanceof sw5e.documents.Item5e ) context.species = details.species;
  ...
  _getLabels() {
    ...
    labels.damageAndHealing = { ...CONFIG.SW5E.damageTypes, ...CONFIG.SW5E.healingTypes };
    return labels;
  }
  ...
  _prepareItem(item, ctx) {
    ...
    if ( item.type === "power" ) {
      ...
      const abbr = {
        action: "SW5E.ActionAbbr",
        bonus: "SW5E.BonusActionAbbr",
        reaction: "SW5E.ReactionAbbr",
        minute: "SW5E.TimeMinuteAbbr",
      ...
      if ( (mode === "always") || (mode === "prepared") ) {
        ...
        ctx.preparation = {
          ...
          cls: prepared ? "active" : "",
          title: isAlways
            ? CONFIG.SW5E.powerPreparationModes.always
            : prepared
              ? CONFIG.SW5E.powerPreparationModes.prepared
    ...
    // Gear
      ...
      if ( "equipped" in system ) {
        ctx.equip = {
          ...
          title: `SW5E.${system.equipped ? "Equipped" : "Unequipped"}`,
  ...
  /**
   * Handle toggling the character's primary powercasting ability.
   * @param {PointerEvent} event  The triggering event.
   * @protected
   */
  ...
  _applyItemTooltips(element) {
    ...
    `;
    element.dataset.tooltipClass = "sw5e2 sw5e-tooltip item-tooltip";
  ...
  _onResize(event) {
    ...
    game.user.setFlag("sw5e", "sheetPrefs.character", { width, height });
  }
