  ...
  async registerModuleArt() {
    ...
    for ( const module of game.modules ) {
      ...
      try {
        ...
        await this.#parseArtMapping(module.id, mapping, flags["sw5e-art-credit"]);
      } catch( e ) {
        console.error(e);
      }
    }
    ...
    // Load system mapping.
    try {
      const mapping = await foundry.utils.fetchJsonWithTimeout("systems/sw5e/json/fa-token-mapping.json");
      const credit = `
        <em>
          Token artwork by
...
export class ModuleArtConfig extends FormApplication {
  ...
  getData(options={}) {
    ...
    context.config.unshift({label: game.system.title, id: game.system.id, ...this.object.sw5e});
    return context;
  }
