...
export default class TokenDocument5e extends SystemFlagsMixin(TokenDocument) {
  ...
  async toggleActiveEffect(effectData, {overlay=false, active}={}) {
    ...
    const id = staticID(`sw5e${effectData.id}`);
  ...
  flashRing({ dhp, dtemp }) {
    let color;
    const options = {};
    if ( dtemp ) color = CONFIG.SW5E.tokenRingColors.temp;
    else if ( dhp > 0 ) color = CONFIG.SW5E.tokenRingColors.healing;
    else if ( dhp < 0 ) {
      color = CONFIG.SW5E.tokenRingColors.damage;
