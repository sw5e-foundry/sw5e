...
export default class Award extends DialogMixin(FormApplication) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["sw5e2", "award", "dialog"],
      template: "systems/sw5e/templates/apps/award.hbs",
      title: "SW5E.Award.Title",
      width: 350,
      height: "auto",
      currency: null,
      xp: null,
  ...
  getData(options={}) {
    ...
    context.currency = Object.entries(CONFIG.SW5E.currencies).reduce((obj, [k, { label }]) => {
    ...
    context.hideXP = game.settings.get("sw5e", "disableExperienceTracking");
    context.noPrimaryParty = !game.settings.get("sw5e", "primaryParty")?.actor && !this.isPartyAward;
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<sw5e-icon class="fa-fw" src="systems/sw5e/icons/svg/backpack.svg"></sw5e-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("sw5e", "awardDestinations", destinations);
  }
  ...
  static async handleAward(message) {
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("sw5e", "awardDestinations");
