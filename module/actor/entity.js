...
export default class Actor5e extends Actor {
  ...
  async usePower(item, {configureDialog=true}={}) {
    ...
    if ( usesSlots && consume && (lvl > 0) ) {
      const slots = parseInt(this.data.data.powers[consume].value);
  ...
  async rollHitDie(denomination) {
    ...
    if ( !cls ) {
      return ui.notifications.error(game.i18n.format("SW5E.HitDiceWarn", {name: this.name, formula: formula}));
    }
