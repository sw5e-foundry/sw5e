...
export default class TokenConfig5e extends TokenConfig {
   ...
   * @type {string}
   */
  static dynamicRingTemplate = "systems/sw5e/templates/apps/parts/dynamic-ring.hbs";
  ...
  async _addTokenRingConfiguration(html) {
    ...
    const flags = this.document.getFlag("sw5e", "tokenRing") ?? {};
    ringTab.innerHTML = await renderTemplate(this.constructor.dynamicRingTemplate, {
      flags,
      effects: Object.entries(CONFIG.SW5E.tokenRings.effects).reduce((obj, [key, label]) => {
    ...
    tab.insertAdjacentHTML("afterbegin", `
      <nav class="tabs sheet-tabs secondary-tabs" data-group="appearance">
        ...
        <a class="item" data-tab="ring" data-group="appearance">
          <i class="fa-solid fa-ring"></i> ${game.i18n.localize("SW5E.TokenRings.Title")}
        </a>
      </nav>
    `);
  ...
  _prepareResourceLabels(html) {
    ...
    for ( const select of html.querySelectorAll('[name="bar1.attribute"], [name="bar2.attribute"]') ) {
      const groups = {
        abilities: makeOptgroup("SW5E.AbilityScorePl", select),
        movement: makeOptgroup("SW5E.MovementSpeeds", select),
        senses: makeOptgroup("SW5E.Senses", select),
        skills: makeOptgroup("SW5E.SkillPassives", select),
        slots: makeOptgroup("JOURNALENTRYPAGE.SW5E.Class.PowerSlots", select)
      };
      ...
      select.querySelectorAll("option").forEach(option => {
        ...
        else if ( option.value.startsWith("powers.") ) groups.slots.appendChild(option);
      });
      ...
      if ( !foundry.utils.isEmpty(items) ) {
        const group = makeOptgroup("SW5E.ConsumeCharges", select);
